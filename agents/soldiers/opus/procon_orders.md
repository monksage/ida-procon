...
```bash
curl -s --noproxy '*' "http://127.0.0.1:40000/next-entry?module={MODULE}&size=precontour&limit=1"
```

If empty, try `size=func`. Pick the first result. You handle the large complex functions (200+ lines).
...
1. Rename variables from `v1, v2` to meaningful names based on context
2. Add a descriptive comment AFTER the header block, before the function signature
3. Do NOT change the logic — this is cleanup, not rewriting
